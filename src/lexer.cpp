...
std::optional<Token> Lexer::next() {
    if (i >= source.length()) return std::nullopt; // End of input

    char ch = source[i];

    // Helper function to handle multi-character operators
    auto handleMultiCharOp = [this](char expectedNextCh, std::string_view op, TokenType singleType, TokenType multiType) -> std::optional<Token> {
        char nextCh = this->peek(1).value_or('\0');
        if (nextCh == expectedNextCh) {
            return makeAndAdvance(std::string(op) + expectedNextCh, multiType);
        }
        return makeAndAdvance(std::string(1, ch), singleType);
    };

    switch (ch) {
        case '+': return handleMultiCharOp('=', "+", TokenType::Plus, TokenType::PlusEquals);
        case '-': return handleMultiCharOp('=', "-", TokenType::Minus, TokenType::MinusEquals);
        case '*': return handleMultiCharOp('=', "*", TokenType::Times, TokenType::TimesEquals);
        case '/': {
            char nextCh = this->peek(1).value_or('\0');
            if (nextCh == '=') {
                return makeAndAdvance("/=", TokenType::DivideEquals);
            } else if (nextCh == '/') {
                return makeAndAdvance("//", TokenType::DoubleSlash); // Handle comments
            }
            return makeAndAdvance("/", TokenType::Divide);
        }
        case '&': return handleMultiCharOp('&', "&", TokenType::Unknown, TokenType::And);
        case '|': return handleMultiCharOp('|', "|", TokenType::Unknown, TokenType::Or);
        case '.': {
            char nextCh = this->peek(1).value_or('\0');
            if (nextCh == '.') {
                char nextNextCh = this->peek(2).value_or('\0');
                if (nextNextCh == '=') {
                    return makeAndAdvance("..=", TokenType::RangeEquals);
                }
                return makeAndAdvance("..", TokenType::Range);
            }
            return makeAndAdvance(".", TokenType::Dot);
        }
        case '!': return handleMultiCharOp('=', "!", TokenType::Bang, TokenType::NotEquals);
        case '=': return handleMultiCharOp('=', "=", TokenType::Assign, TokenType::EqualsEquals);
        case '<': return handleMultiCharOp('=', "<", TokenType::LessThan, TokenType::LessThanOrEquals);
        case '>': return handleMultiCharOp('=', ">", TokenType::GreaterThan, TokenType::GreaterThanOrEquals);
        case '(': return makeAndAdvance("(", TokenType::L_Paren);
        case ')': return makeAndAdvance(")", TokenType::R_Paren);
        case '{': return makeAndAdvance("{", TokenType::L_Brace);
        case '}': return makeAndAdvance("}", TokenType::R_Brace);
        case '[': return makeAndAdvance("[", TokenType::L_Bracket);
        case ']': return makeAndAdvance("]", TokenType::R_Bracket);
        case ';': return makeAndAdvance(";", TokenType::Semicolon);
        default: return std::nullopt; // Unknown character
    }
}
*/
